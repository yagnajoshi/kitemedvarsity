...
export class CoreCoursesCourseProgressComponent implements OnInit, OnDestroy {
    ...
    constructor(@Optional() private navCtrl: NavController, private courseHelper: CoreCourseHelperProvider,
            ...
            private popoverCtrl: PopoverController, private userProvider: CoreUserProvider, private textUtils: CoreTextUtilsProvider) { }
    ...
    openCourse(course: any): void {
        ...
        /*--------------Yagna Added Code Start----------------*/
        /* Verify first that the particular course is active for the user before
        */
        ...
        console.log('Check Courses Test variables', dataSuspend);        
         site.read('local_suspension_courseprogress', dataSuspend, preSets).then((result) => {
            const data = this.textUtils.parseJSON(result.result);
             const SuspendResponse = data[0];
            console.log('Check Courses status', SuspendResponse.enrollment_status );
           // return { enrollment_status: SuspendResponse.enrollment_status };
           if( SuspendResponse.enrollment_status == '0') {
                this.courseHelper.openCourse(this.navCtrl, course);
           }
           else {
               let msg = "Your access to course <B>"+course.displayname + "</B> is Deactivated / Under Enrollment Process, for further details please write to <a href=\"mailto:support@medvarsity.com\">support@medvarsity.com</a> or wait for some time. Once verified you can access course.";
                this.domUtils.showAlertTranslated("Alert", msg, "Close", 2*60*1000);
           }
           
        });
        /*--------------Yagna Added Code End----------------*/
