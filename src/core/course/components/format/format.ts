...
export class CoreCourseFormatComponent implements OnInit, OnChanges, OnDestroy {
    ...
    constructor(private cfDelegate: CoreCourseFormatDelegate, translate: TranslateService, private injector: Injector,
            ...
            private courseProvider: CoreCourseProvider,private platform: Platform) {
    ...
    lastOpendModule;
    moduleClicked(currentCourse: any, clikedModule:any): void {
        console.log("Item Cliked" + currentCourse);
        console.log("Module" +this.course);
        this.lastOpendModule = clikedModule;

        if(this.lastOpendModule.completiondata.state == 0) {
            this.StartTimer(currentCourse.recentactivity);      
        }
    }
    ...
    this.cfDelegate.completionTimer = setTimeout(x => 
    {   
        if(this.cfDelegate.isModuleDescriptionActive) {        
            console.log("Module"+this.lastOpendModule.completiondata);
            const site = this.sitesProvider.getCurrentSite();
           this.completion = this.lastOpendModule.completiondata;
           this.completionClicked()            
        }
    }, timeOut);
  }
