...
export class CoreCourseModuleComponent implements OnInit, OnDestroy {
    ...
    moduleClicked(event: Event): void {
        if(this.module.uservisible !== false && this.module.modname === 'simplecertificate') {
            this.sitesProvider.getCurrentSite().openInAppWithAutoLogin(this.module.url);
        }
        else if (this.module.uservisible !== false && this.module.handlerData.action) {
