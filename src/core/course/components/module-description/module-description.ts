...
export class CoreCourseModuleDescriptionComponent implements OnInit, OnDestroy{
    ...
    isStartedFromAnotherClass: boolean = false;

    constructor(private cfDelegate: CoreCourseFormatDelegate) {
        // Nothing to do.
    }

    ngOnInit (){
        if(this.cfDelegate.isModuleDescriptionActive) {
            this.isStartedFromAnotherClass = true;
        }
        else {
             this.cfDelegate.isModuleDescriptionActive = true;
        }    
    }

    ngOnDestroy (){
        if(!this.isStartedFromAnotherClass) {
        this.cfDelegate.isModuleDescriptionActive = false;
        clearTimeout(this.cfDelegate.completionTimer);
        }
